...
namespace ImGui {
    ...
    IMGUI_API void  RenderNeoSequencerTopBarOverlay(float zoom, float valuesWidth,unsigned startFrame, unsigned endFrame, unsigned offsetFrame, const ImVec2 &cursor, const ImVec2& size, ImDrawList * drawList = nullptr, bool drawFrameLines = true, bool drawFrameText = true, float maxPixelsPerTick = -1.0f);
    ...
    IMGUI_API float GetPerFrameWidth(float totalSizeX, float valuesWidth, unsigned endFrame, unsigned startFrame, float zoom);
}
